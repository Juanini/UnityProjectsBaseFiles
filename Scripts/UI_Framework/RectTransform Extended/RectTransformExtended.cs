...
public class RectTransformExtendedInspector : Editor
{
    ...
    {
        EditorGUILayout.LabelField("This component is editor only.");
        EditorGUILayout.LabelField("Be sure you remove it, once you got the correct values to operate.");
    ...
    {
        var t = target as RectTransformExtended;
        t.GetReferences();
        t.UpdateAndApply();
    }
    ...
    private void ShowScaleWarning()
    {
        ...
        if(!rt.localScale.IsAproximately(Vector3.one))
            EditorGUILayout.HelpBox("The scale of the object is not 1. This causes unexpected results since the values does not represent the real visual size anymore.", MessageType.Warning);
    }
